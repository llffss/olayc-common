 ...
 * @date 2018-08-15
 */
public class OlaBeanUtils extends org.apache.commons.beanutils.BeanUtils {

	private static <T> T copyItSelf(T orig, String feilds) {
		...
		try {
			...
			Class aClass1 = orig.getClass();

			target = (T) aClass1.newInstance();

			// Copy the properties, converting as necessary
			PropertyDescriptor[] origDescriptors = PropertyUtils.getPropertyDescriptors(orig);
			String[] split = feilds.split("-");
			for (String name : split) {
				if (PropertyUtils.isReadable(orig, name) && PropertyUtils.isWriteable(target, name)) {
